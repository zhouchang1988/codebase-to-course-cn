...
**普通版：** 每个屏幕必须至少 50% 是视觉内容（图表、代码块、问卷、动画）。
**专业版：** 注重信息密度，可以文字占比更高，但必须有足够的图表和动画。
...
**普通版：** 学习者应该能够打开真实文件并看到完全相同的代码。不要通过编辑代码来使其更短，而是从代码库中*选择*自然简短（5-10 行）的片段来说明要点。
**专业版：** 使用真实代码，仅保留必要的上下文（导入、类型注解），但保持核心逻辑完整。
...
**普通版每个模块至少：** 测验、数据流动画、群聊、架构图、拖放之一
**专业版每个模块至少：** 架构图、数据流动画、时序图之一
